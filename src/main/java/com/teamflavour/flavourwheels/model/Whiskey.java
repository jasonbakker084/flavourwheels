package com.teamflavour.flavourwheels.model;

import com.fasterxml.jackson.annotation.JsonProperty;
import org.apache.tomcat.util.codec.binary.Base64;
import org.hibernate.annotations.OnDelete;
import org.hibernate.annotations.OnDeleteAction;
import org.springframework.format.annotation.DateTimeFormat;

import javax.persistence.*;
import java.sql.Blob;
import java.sql.SQLException;
import java.util.Collection;
import java.util.Date;
import java.util.Objects;

@Entity
@Table(name = "whiskeyLib")
public class Whiskey {

    @Id
    @GeneratedValue(strategy = GenerationType.AUTO)
    private Long id;
    //    @GeneratedValue(strategy = GenerationType.AUTO)
//    private String idF;
    @GeneratedValue(strategy = GenerationType.AUTO)
    @Column(name = "date")
    private Date date;
    @Column(name = "whiskeyName")
    private String whiskeyName;
    @Column(name = "whiskeyHouse")
    private String whiskeyHouse;
    @Column(name = "whiskeyColor")
    private String whiskeyColor;
    @Column(name = "whiskeyYear")
    @DateTimeFormat(pattern = "dd-MM-yyyy")
    private String whiskeyYear;
    @Column(name = "processingMethod")
    private String processingMethod;
    @Column(name = "tastingMethod")
    private String tastingMethod;
    @Column(name = "grapeType")
    private String grapeType;
    @Column(name = "userNotes")
    private String userNotes;
    @Column(name = "flag")
    private Boolean flag;
    @Column(name = "file")
    @Lob
    private Blob file;
    @Column(name = "filetype")
    private String fileType;
    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(name = "user_id", nullable = false)
    @OnDelete(action = OnDeleteAction.CASCADE)
    private User user;

    @ManyToMany(fetch = FetchType.EAGER)
    @JoinTable(
            name = "whiskey_flavors",
            joinColumns = @JoinColumn(
                    name = "whiskey_id", referencedColumnName = "id"),
            inverseJoinColumns = @JoinColumn(
                    name = "flavorWhiskey_id", referencedColumnName = "id"))
    private Collection<FlavorWhiskey> flavorWhiskeys;

    public Whiskey() {
        super();
    }

    public Whiskey(@JsonProperty("whiskeyName") String whiskeyName,
                   @JsonProperty("whiskeyHouse") String whiskeyHouse,
                @JsonProperty("whiskeyColor")String whiskeyColor,
                   @JsonProperty("whiskeyYear") String whiskeyYear,
                   @JsonProperty("tastingMethod") String tastingMethod,
                @JsonProperty("grapeType") String grapeType,
                   @JsonProperty("processingMethod") String processingMethod,
                @JsonProperty("userNotes") String userNotes)//, @JsonProperty("file") MultipartFile file)
    {
        super();
        this.date = new Date(System.currentTimeMillis());
        this.whiskeyName = whiskeyName;
        this.whiskeyHouse = whiskeyHouse;
        this.whiskeyColor = whiskeyColor;
        this.whiskeyYear = whiskeyYear;
        this.processingMethod = processingMethod;
        this.tastingMethod = tastingMethod;
        this.grapeType = grapeType;
        this.userNotes = userNotes;
        //  this.file = file;
    }


    public Whiskey(Date date, String whiskeyName, String whiskeyHouse, String whiskeyColor, String whiskeyYear, String processingMethod, String tastingMethod, String grapeType, String userNotes, Boolean flag,
                Blob file)
    {
        super();
        this.date = date;
        this.whiskeyName = whiskeyName;
        this.whiskeyHouse = whiskeyHouse;
        this.whiskeyColor = whiskeyColor;
        this.whiskeyYear = whiskeyYear;
        this.processingMethod = processingMethod;
        this.tastingMethod = tastingMethod;
        this.grapeType = grapeType;
        this.userNotes = userNotes;
        this.flag = flag;
        this.file = file;
    }

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public String getWhiskeyName() {
        return whiskeyName;
    }

    public void setWhiskeyName(String whiskeyName) {
        this.whiskeyName = whiskeyName;
    }

    public String getWhiskeyHouse() {
        return whiskeyHouse;
    }

    public void setWhiskeyHouse(String whiskeyHouse) {
        this.whiskeyHouse = whiskeyHouse;
    }

    public String getWhiskeyColor() {
        return whiskeyColor;
    }

    public void setWhiskeyColor(String whiskeyColor) {
        this.whiskeyColor = whiskeyColor;
    }

    public String getProcessingMethod() {
        return processingMethod;
    }

    public void setProcessingMethod(String processingMethod) {
        this.processingMethod = processingMethod;
    }

    public String getTastingMethod() {
        return tastingMethod;
    }

    public void setTastingMethod(String tastingMethod) {
        this.tastingMethod = tastingMethod;
    }

    public String getGrapeType() {
        return grapeType;
    }

    public void setGrapeType(String grapeType) {
        this.grapeType = grapeType;
    }

    public Date getDate() { return date; }

    public void setDate(Date date) { this.date = date; }

    public String getUserNotes() { return userNotes; }

    public void setUserNotes(String userNotes) { this.userNotes = userNotes; }

    public Boolean isFlag() {
        return flag;
    }

    public void setFlag(Boolean flag) {
        this.flag = flag;
    }

    public Blob getFile() {
        return file;
    }

    public void setFile(Blob file) {
        this.file = file;
    }

    public void setUser(User user) {
        this.user = user;
    }

    public Collection<FlavorWhiskey> getFlavorWhiskeys() {
        return flavorWhiskeys;
    }

    //flavorWhiskeys opgesomd
    public String getFlavorString() {
        String result = "";
        for (FlavorWhiskey f: flavorWhiskeys) {
            result += f.getName() + ", ";
        }
        if (result.length() > 1){
            result = result.substring(0,result.length()-2);
        }
        return result;
    }

    public void setFlavorWhiskeys(Collection<FlavorWhiskey> flavorWhiskeys) {
        this.flavorWhiskeys = flavorWhiskeys;
    }

    public String getWhiskeyYear() { return whiskeyYear; }

    public void setWhiskeyYear(String whiskeyYear) { this.whiskeyYear = whiskeyYear; }

    public String getFileType() {
        return fileType;
    }

    public void setFileType(String fileType) {
        this.fileType = fileType;
    }

    public String getImageString() throws SQLException {
        if (file == null){
            return "data:image/png;base64,";
        }
        return "data:image/"+ fileType + ";base64," + Base64.encodeBase64String(file.getBytes(1, (int) file.length()));
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) return true;
        if (o == null || getClass() != o.getClass()) return false;
        Whiskey whiskey = (Whiskey) o;
        return Objects.equals(id,
                whiskey.id) &&
                Objects.equals(whiskeyName, whiskey.whiskeyName) &&
                Objects.equals(whiskeyHouse, whiskey.whiskeyHouse) &&
                Objects.equals(whiskeyColor, whiskey.whiskeyColor) &&
                Objects.equals(processingMethod, whiskey.processingMethod) &&
                Objects.equals(tastingMethod, whiskey.tastingMethod) &&
                Objects.equals(date, whiskey.date) &&
                Objects.equals(grapeType, whiskey.grapeType) &&
                Objects.equals(userNotes, whiskey.userNotes) &&
                Objects.equals(flag, whiskey.flag) &&
                Objects.equals(file, whiskey.file);
    }

    @Override
    public int hashCode() {
        return Objects.hash(id, date, whiskeyName, whiskeyHouse, whiskeyColor, whiskeyYear, processingMethod, tastingMethod, grapeType, userNotes, flag);
    }

    @Override
    public String toString() {
        final StringBuilder sb = new StringBuilder("Whiskey Library{");
        sb.append("id=").append(id);
        sb.append(", date='").append(date).append('\'');
        sb.append(", whiskeyName='").append(whiskeyName).append('\'');
        sb.append(", whiskeyHouse='").append(whiskeyHouse).append('\'');
        sb.append(", whiskeyColor='").append(whiskeyColor).append('\'');
        sb.append(", whiskeyYear='").append(whiskeyYear).append('\'');
        sb.append(", processingMethod='").append(processingMethod).append('\'');
        sb.append(", tastingMethod='").append(tastingMethod).append('\'');
        sb.append(", grapeType='").append(grapeType).append('\'');
        sb.append(", userNotes='").append(userNotes).append('\'');
        sb.append('}');
        return sb.toString();
    }
}